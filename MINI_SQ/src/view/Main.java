package view;



import java.util.Random;
import java.util.Scanner;

import controller.MbController;
import controller.Mp3;
import controller.UserController;
import controller.gameController;
import javazoom.jl.player.MP3Player;
import model.MbDTO;
import model.UserDAO;
import model.UserDTO;

public class Main {

	public static void main(String[] args) throws InterruptedException {

		Scanner sc = new Scanner(System.in);
		MbDTO dto = null;
		MbController con = new MbController();
		gameController gcon = new gameController();
		UserController ucon = new UserController();
		UserDTO udto = null;





		MP3Player mp3 = new MP3Player();
		
		Mp3 m = new Mp3();
		
		Random rd = new Random();

		// 회원가입 및 로그인
		Thread.sleep(3000);
		System.out.println(
	            "                                                            •••   •••                  \r\n"
	           + "                                                           ••••   ••••                  \r\n"
	           + "                                                           •••     •••                \r\n"
	           + "                                                          ••••      •••                \r\n"
	           + "                                                          •••       •••                \r\n"
	           + "                                                         •••         •••               \r\n"
	           + "                                                       ••••           ••••              \r\n"
	           + "                                                       ••••••••••••••••••••              \r\n"
	           + "                                                      •••••••••••••••••••••             \r\n"
	           + "                                                      •••••••••••••••••••••             \r\n"
	           + "                                                                     \r\n"
	           + "                                                                     \r\n"
	           + "•\t\t  ██████████████     ██████████  ███████   █████████████████████████  \r\n"
	           + "••••\t\t ███░░░░░░░░░░███  ███░░░░░░░███░░░███   ░░░███░░███░░███░░░░░░░░ ███ \r\n"
	           + "•••••\t\t░███        ░░███ ███      ░░░███░░███     ░███ ░███ ░███       ░░░███\r\n"
	           + "••••••\t\t░███        ░░░░  ███      ░░░███░░███     ░███ ░███ ░███        ░░████\r\n"
	           + "  •••••\t\t░███              ███      ░░░███░░███     ░███ ░███ ░███         ░████\r\n"
	           + "   ••••\t\t░░██████████████ ░███        ░███░░███     ░███ ░███ ░███         ░████\r\n"
	           + "    •••\t\t ░░░░░░░░░░░░░███░███    ████░███░░███     ░███ ░███ ░███         ░████\r\n"
	           + "    •••\t\t ███         ░███░███     ░██████░░███     ░███ ░███ ░███         ░████\r\n"
	           + "    •••\t\t ███         ░███░███     ░░█████░░███     ░███ ░███ ░███         ░███ \r\n"
	           + "    •••\t\t  ██         ░███░ ███     ░░███ ░░███     ░███ ░███ ░███        ░███ \r\n"
	           + "    •••\t\t░░██████████████  ░░░████████░███░░░██████████  ████████████████████  \r\n"
	           + "   •••• \t\t ░░░░░░░░░░░░░░     ░░░░░░░░ ░░░  ░░░░░░░░░░░  ░░░░░░░░░░░░░░░░░░ \r\n" //12
	           + "   ••••\t\t                                                           \r\n"
	           + "   ••••\t\t                                                           \r\n"
	           + "   ••••\t\t                                오징어 게임                           \r\n"
	           + " •••••\t\t                                                            \r\n"
	           + "•••••\t\t                                                      \r\n"
	           + "••••\t\t                ████████████   ████████████  ████████   ██████████████████████\r\n"
	           + "••\t\t               ███░░░░░░░░███ ███░░░░░░░░███░░████████ ████████░░███░░░░░░░░░█\r\n"
	           + "\t\t              ███░░░░░░░░░░  ░███░░░░░░░░███ ░███░░░█████░░░███░░███        ░ \r\n"
	           + "\t\t             ░███       ░░░░ ░███      ░░███ ░███░░░█████░░░███ ░███      █        \r\n"
	           + "\t\t             ░███            ░███      ░░███ ░███░░░█████░░░███ ░██████████   \r\n"
	           + "\t\t             ░███            ░██████████████ ░███░░░░███   ░███ ░██████████   \r\n"
	           + "\t\t             ░███      ██████░███░░░░░░░░███ ░███░░░░███   ░███ ░███░░░░░░█    \r\n"
	           + "\t\t             ░███      ░░███ ░███░░░░░░░░███ ░███ ░░░░░    ░███ ░███     ░   \r\n"
	           + "\t\t             ░███      ░░███░░███       ░███ ░███          ░███ ░███     ░  ░█\r\n"
	           + "\t\t             ░░███     ░░███ ░███       ░███ ░███          ░███ ░███     ░  ░█\r\n"
	           + "\t\t              ░░████████████ █████      ████████████       ███████████████████\r\n"
	           + "\t\t               ░░░░░░░░░░░░ ░░░░░     ░░░░░░░░░░░░░       ░░░░░░░░░░░░░░░░░░░ \r\n"
	           + "                        •••••••••••••••••••••                                              \r\n"                
	              + "                        •••••••••••••••••••••                                                  \r\n"
	              + "                        •••••••••••••••••••••                                                \r\n"
	              + "                        •••••          ••••••                                                  \r\n"
	              + "                        ••••           ••••••                                                  \r\n"
	              + "                        ••••           ••••••                                          ");
		 while (true) {
			m.fx(2);
			Thread.sleep(5000);
	         System.out.println("\n\n\n\n\t                                        ●                                                       ");
	         Thread.sleep(2000);
	         System.out.println("\t                                        ▲                                                       ");
	         Thread.sleep(2000);
	         System.out.println("\t                                        ■                                                       ");
	         Thread.sleep(2000);
	         System.out.println("\t                                        $                                                       \n\n\n\n\n");
	         Thread.sleep(2000);
	         System.out.println("                                                   __  __________   ____  __                                                          \r\n"
	               + "                                                  /  |/  / ____/ | / / / / /                                                          \r\n"
	               + " ________________________________________________/ /|_/ / __/ /  |/ / / / /___________________________________________________________\r\n"
	               + "/_____/_____/_____/_____/_____/_____/_____/_____/ /  / / /___/ /|  / /_/ /_____/_____/_____/_____/_____/_____/_____/_____/_____/_____/\r\n"
	               + "                                               /_/  /_/_____/_/ |_/\\____/                                                             \r\n"
	               + "                                                                                                                                      ");
	         Thread.sleep(2000);
	         System.out.print("[1]회원가입 \t\t\t\t[2]로그인 \t\t\t\t[3]종료 >> \r\n\n");
	         int select = sc.nextInt(); // 사용자가 선택

			if (select == 1) {
				// (insert)
				// 사용자로부터 ID,PW을 입력받아 회원가입

				System.out.print("ID : ");
				String id = sc.next();
				System.out.print("PW : ");
				String pw = sc.next();

				dto = new MbDTO(id, pw);
				con.insert(dto);

			} else if (select == 2) {
				// login

				System.out.print("ID : ");
				String id = sc.next();
				System.out.print("PW : ");
				String pw = sc.next();

				con = new MbController();
				dto = new MbDTO(id, pw);

				// 참가자 등록 (닉네임, 등번호 입력)
	            
	            if (con.login(dto) == true && ucon.selectOne(id)==null) {
	               System.out.print("닉네임을 입력해주세요 : ");
	               String name = sc.next();
	               System.out.print("등번호를 입력해주세요 : ");
	               int backnum = sc.nextInt();

	               ucon = new UserController();
	               udto = new UserDTO(id, name, backnum);
	               new UserDAO().user(udto);
	               ucon.userSel(id);

	            } else {
	               ucon.userSel(id);
	             
	            }

	            String n=new UserDAO().selectOne(id).getName(); // 닉네임
	            int d=new UserDAO().selectOne(id).getBacknum(); // 등번호
	            
				// 게임시작
//				System.out.println("게임이 시작 됩니다.");
	            m.stp(2);
	            Thread.sleep(4000);
	            System.out.println("\n\n\n");
	            
	            Thread.sleep(1000);
	            System.out.println("╔══════════════════════════════════════════════════════════════╗ \r\n");
	            System.out.println("\t30살이 되도록 백수인 나....." + n + "\n"  ); 
	            Thread.sleep(1000);
	            System.out.println("\t엄마에게 용돈 좀 달라고 했다가 \n\t결국 대판 싸웠고 나왔다. \n");
	            Thread.sleep(1000);
	            System.out.println("\t어디론가 떠나고 싶어 무작정 지하철을 타러 왔지만.....\n");
	            System.out.println("\t막상 갈 곳도 돈도 없다. \n");
	            System.out.println("\t그렇게 한숨만 푹푹 내쉬고 있는데..... \n");
	            Thread.sleep(1000);
	            m.fx(8);
	            Thread.sleep(3000);
	            System.out.println("\t갑자기 한 남자가 다가온다.\n");
	            m.fx(9);
	            Thread.sleep(2000);
	            System.out.println("\t⌜ 저와 .. 게임 한판 하시렵니까..? ⌟\n");
	            Thread.sleep(1000);
	            System.out.println("\t⌜ 네..? ⌟\n");
	            Thread.sleep(1000);
	            System.out.println("\t⌜ 딱지를 넘기시면 10만원... ⌟\n");
	            Thread.sleep(1000);
	            System.out.println("\t⌜ ...못 넘기시면 싸다구 1대..어떠세요? ⌟\n");
	            Thread.sleep(3000);
	            System.out.println("\t돈도 없고 갈 곳도 없는 내 인생 두려울 게 뭐 있으랴 기꺼이 제안에 응해본다.\n");
	            System.out.println("╚═════════════════════════════════════════════════════════════════╝ ");
	            		
	            
	            
	            	System.out.println("\n");
	            	System.out.println("\n");
	            	System.out.println("\n");
	            	System.out.println("\n");
	            	Thread.sleep(1000);
	            	System.out.println("\n");
	            	Thread.sleep(1000);
	            	System.out.println("\n");
	            	Thread.sleep(1000);
	            	System.out.println("\n");
	            	Thread.sleep(1000);
	            	System.out.println("\n");
	            	Thread.sleep(1000);
	            	System.out.println("\n");
	            	Thread.sleep(3000);
	               System.out.println(""
	               		+ "                    /$$$$$$   /$$$$$$  /$$      /$$ /$$$$$$$$        /$$$$$$  /$$$$$$$$ /$$$$$$  /$$$$$$$  /$$$$$$$$               \r\n"
	               		+ "                   /$$__  $$ /$$__  $$| $$$    /$$$| $$_____/       /$$__  $$|__  $$__//$$__  $$| $$__  $$|__  $$__/               \r\n"
	               		+ " /$$$$ /$$$$ /$$$$| $$  \\__/| $$  \\ $$| $$$$  /$$$$| $$            | $$  \\__/   | $$  | $$  \\ $$| $$  \\ $$   | $$ /$$$$ /$$$$ /$$$$\r\n"
	               		+ "|____/|____/|____/| $$ /$$$$| $$$$$$$$| $$ $$/$$ $$| $$$$$         |  $$$$$$    | $$  | $$$$$$$$| $$$$$$$/   | $$|____/|____/|____/\r\n"
	               		+ " /$$$$ /$$$$ /$$$$| $$|_  $$| $$__  $$| $$  $$$| $$| $$__/          \\____  $$   | $$  | $$__  $$| $$__  $$   | $$ /$$$$ /$$$$ /$$$$\r\n"
	               		+ "|____/|____/|____/| $$  \\ $$| $$  | $$| $$\\  $ | $$| $$             /$$  \\ $$   | $$  | $$  | $$| $$  \\ $$   | $$|____/|____/|____/\r\n"
	               		+ "                  |  $$$$$$/| $$  | $$| $$ \\/  | $$| $$$$$$$$      |  $$$$$$/   | $$  | $$  | $$| $$  | $$   | $$                  \r\n"
	               		+ "                   \\______/ |__/  |__/|__/     |__/|________/       \\______/    |__/  |__/  |__/|__/  |__/   |__/                  \r\n"
	               		+ "                                                                                                                                   \r\n"
	               		+ "                                                             게임 스타트                                                              \r\n"
	               		+ "                                                                                                                                   ");
	               System.out.println("\n");
	               Thread.sleep(1000);
	               System.out.println("\n");
	               Thread.sleep(1000);
	               System.out.println("\n");
	               Thread.sleep(1000);
	               System.out.println("\n");
	               Thread.sleep(1000);
	               System.out.println("\n");
	               Thread.sleep(1000);
	               System.out.println("\n");
	               Thread.sleep(1000);
	               System.out.println("\n");
	               Thread.sleep(1000);
	               System.out.println("\n");
	               Thread.sleep(1000);
	               System.out.println("\n");
	               Thread.sleep(1000);
	               System.out.println("\n");
	               Thread.sleep(1000);
	               
	               
	               System.out.println("\n\n"
	                       + " ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______\r\n"
	                         + "/_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____/\n\n\n\n\n\n");
				m.stp(8);
				// 1. 딱지 치기
				Thread.sleep(4000);
				System.out.println("\n\n\n\n\n\n");
				System.out.println(" \t\t\t\t/$$$$$$$  /$$$$$$$   /$$$$$$  /$$   /$$    /$$$$$ /$$$$$$\r\n"
						+ "\t\t\t\t| $$__  $$| $$__  $$ /$$__  $$| $$  /$$/   |__  $$|_  $$_/\r\n"
						+ "\t\t\t\t| $$  \\ $$| $$  \\ $$| $$  \\ $$| $$ /$$/       | $$  | $$  \r\n"
						+ "\t\t\t\t| $$  | $$| $$  | $$| $$$$$$$$| $$$$$/        | $$  | $$  \r\n"
						+ "\t\t\t\t| $$  | $$| $$  | $$| $$__  $$| $$  $$   /$$  | $$  | $$  \r\n"
						+ "\t\t\t\t| $$  | $$| $$  | $$| $$  | $$| $$\\  $$ | $$  | $$  | $$  \r\n"
						+ "\t\t\t\t| $$$$$$$/| $$$$$$$/| $$  | $$| $$ \\  $$|  $$$$$$/ /$$$$$$\r\n"
						+ "\t\t\t\t|_______/ |_______/ |__/  |__/|__/  \\__/ \\______/ |______/\r\n"
						+ "\t\t\t\t                                                          \r\n"
						+ "\t\t\t\t                        딱지치기                             \r\n"
						+ "\t\t\t\t                                                          \r\n"
						+ "\t\t\t\t        /$$$$$$   /$$$$$$  /$$      /$$ /$$$$$$$$         \r\n"
						+ "\t\t\t\t       /$$__  $$ /$$__  $$| $$$    /$$$| $$_____/         \r\n"
						+ "\t\t\t\t      | $$  \\__/| $$  \\ $$| $$$$  /$$$$| $$               \r\n"
						+ "\t\t\t\t      | $$ /$$$$| $$$$$$$$| $$ $$/$$ $$| $$$$$            \r\n"
						+ "\t\t\t\t      | $$|_  $$| $$__  $$| $$  $$$| $$| $$__/            \r\n"
						+ "\t\t\t\t      | $$  \\ $$| $$  | $$| $$\\  $ | $$| $$               \r\n"
						+ "\t\t\t\t      |  $$$$$$/| $$  | $$| $$ \\/  | $$| $$$$$$$$         \r\n"
						+ "\t\t\t\t       \\______/ |__/  |__/|__/     |__/|________/        \r\n"
						+ "\t\t\t\t                                                          \r\n"
						+ "\t\t\t\t                                                          \r\n"
						+ "\t\t\t\t                                                          ");
				System.out.println(
		                   " ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______\r\n"
		                   + "/_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____/");
				gcon.ddak();
				System.out.println("\n\n\n\n");
				
				// 2. 무궁화 꽃이 피었습니다.
				Thread.sleep(2000);
				Thread.sleep(2000);
	            System.out.println("╔══════════════════════════════════════════════════════╗ \r\n"
	                  + "\t목숨을 건 오징어게임에 오신것을 환영합니다.\r\n"
	                  + "\t게임 시작 후, 클리어하시게 되면 상금을 얻게됩니다.\r\n"
	                  + "\t이번 게임은 무궁화 꽃이 피었습니다 입니다.\n"
	                  + "╚═══════════════════════════════════════════════════════╝ ");
				
				Thread.sleep(3000);
				m.fx(22);
				Thread.sleep(7500);
				m.stp(22);
				System.out.println("\n\n\n");
				System.out.println("\n\n"
		                 + " ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______\r\n"
		                   + "/_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____/\n\n\n\n\n");
				System.out.println(" ██████╗  █████╗ ███╗   ███╗███████╗     ██████╗ ███████╗    ███████╗████████╗ █████╗ ████████╗██╗   ██╗███████╗███████╗\r\n"
						+ "██╔════╝ ██╔══██╗████╗ ████║██╔════╝    ██╔═══██╗██╔════╝    ██╔════╝╚══██╔══╝██╔══██╗╚══██╔══╝██║   ██║██╔════╝██╔════╝\r\n"
						+ "██║  ███╗███████║██╔████╔██║█████╗      ██║   ██║█████╗      ███████╗   ██║   ███████║   ██║   ██║   ██║█████╗  ███████╗\r\n"
						+ "██║   ██║██╔══██║██║╚██╔╝██║██╔══╝      ██║   ██║██╔══╝      ╚════██║   ██║   ██╔══██║   ██║   ██║   ██║██╔══╝  ╚════██║\r\n"
						+ "╚██████╔╝██║  ██║██║ ╚═╝ ██║███████╗    ╚██████╔╝██║         ███████║   ██║   ██║  ██║   ██║   ╚██████╔╝███████╗███████║\r\n"
						+ " ╚═════╝ ╚═╝  ╚═╝╚═╝     ╚═╝╚══════╝     ╚═════╝ ╚═╝         ╚══════╝   ╚═╝   ╚═╝  ╚═╝   ╚═╝    ╚═════╝ ╚══════╝╚══════╝\r\n"
						+ "													무궁화 꽃이 피었습니다	                                                       	");
				System.out.println("\n\n"
		                 + " ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______\r\n"
		                   + "/_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____/");
				 
				gcon.mugung();



				// 달고나
				
				Thread.sleep(4000);
				Thread.sleep(2000);
				m.fx(20);
//				System.out.println("이번 게임은 달고나 게임 입니다.");
				
				System.out.println("\n\n\n");
				System.out.println("\n\n"
		                 + " ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______\r\n"
		                   + "/_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____/\n\n\n\n\n\n");
				System.out.println("██████╗  █████╗ ██╗      ██████╗  ██████╗ ███╗   ██╗ █████╗ \r\n"
	                  + "██╔══██╗██╔══██╗██║     ██╔════╝ ██╔═══██╗████╗  ██║██╔══██╗\r\n"
	                  + "██║  ██║███████║██║     ██║  ███╗██║   ██║██╔██╗ ██║███████║\r\n"
	                  + "██║  ██║██╔══██║██║     ██║   ██║██║   ██║██║╚██╗██║██╔══██║\r\n"
	                  + "██████╔╝██║  ██║███████╗╚██████╔╝╚██████╔╝██║ ╚████║██║  ██║\r\n"
	                  + "╚═════╝ ╚═╝  ╚═╝╚══════╝ ╚═════╝  ╚═════╝ ╚═╝  ╚═══╝╚═╝  ╚═╝\r\n"
	                  + "                                                            \r\n"
	                  + "                             ██████╗  █████╗ ███╗   ███╗███████╗\r\n"
	                  + "       달                    ██╔════╝ ██╔══██╗████╗ ████║██╔════╝\r\n"
	                  + "         고                   ██║  ███╗███████║██╔████╔██║█████╗  \r\n"
	                  + "           나                 ██║   ██║██╔══██║██║╚██╔╝██║██╔══╝  \r\n"
	                  + "             뽑              ╚██████╔╝██║  ██║██║ ╚═╝ ██║███████╗\r\n"
	                  + "               기             ╚═════╝ ╚═╝  ╚═╝╚═╝     ╚═╝╚══════╝\r\n"
	                  + "                                                                ");
				System.out.println("\n\n"
		                 + " ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______\r\n"
		                   + "/_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____/\n\n\n\n\n\n\n");

				Thread.sleep(12000);
				m.stp(20);
	            System.out.println("▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀\n"
	            				+ "\n\t\t다음 달고나를 잘 자르세요. (w를 눌러 시작하세요)\n"
	            				+ "\n▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀▄▀ ");
	            System.out.println("\n\n");
				System.out.println(" ┎─────┒ ");
				System.out.println(" ┃/////┃ ");
				System.out.println(" ┃/////┃ ");
				System.out.println("●┖─────┚ ");
				
				
				
				
				gcon.dal();

				// 5. 징검다리

				Thread.sleep(5000);
				m.fx(23);
//				System.out.println("이번 게임은 징검다리 게임입니다.");
				System.out.println("\n\n"
		                 + " ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______\r\n"
		                   + "/_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//");
				Thread.sleep(2000);
	            System.out.println("\n\n"+
	            		"  █████████  █████                                  ███                    \r\n"
	                  + " ███░░░░░███░░███                                  ░░░                     \r\n"
	                  + "░███    ░░░ ███████    ██████  ████████  ████████  ████ ████████    ███████\r\n"
	                  + "░░█████████░░░███░    ███░░███░░███░░███░░███░░███░░███░░███░░███  ███░░███\r\n"
	                  + " ░░░░░░░░███ ░███    ░███████  ░███ ░███ ░███ ░███ ░███ ░███ ░███ ░███ ░███\r\n"
	                  + " ███    ░███ ░███ ███░███░░░   ░███ ░███ ░███ ░███ ░███ ░███ ░███ ░███ ░███\r\n"
	                  + "░░█████████  ░░█████ ░░██████  ░███████  ░███████  █████████ █████░░███████\r\n"
	                  + " ░░░░░░░░░    ░░░░░   ░░░░░░   ░███░░░   ░███░░░  ░░░░░░░░░ ░░░░░  ░░░░░███\r\n"
	                  + "                               ░███      ░███                      ███ ░███\r\n"
	                  + "                               █████     █████                    ░░██████ \r\n"
	                  + "                              ░░░░░     ░░░░░                      ░░░░░░  \r\n"
	                  + "        █████████  █████         징검다리 건너기             \r\n"
	                  + "       ███░░░░░███░░███                                 \r\n"
	                  + "      ░███    ░░░ ███████    ██████  ████████    ██████ \r\n"
	                  + "      ░░█████████░░░███░    ███░░███░░███░░███  ███░░███\r\n"
	                  + "       ░░░░░░░░███ ░███    ░███ ░███ ░███ ░███ ░███████ \r\n"
	                  + "       ███    ░███ ░███ ███░███ ░███ ░███ ░███ ░███░░░  \r\n"
	                  + "      ░░█████████  ░░█████ ░░██████  ████ █████░░██████ \r\n"
	                  + "       ░░░░░░░░░    ░░░░░   ░░░░░░  ░░░░ ░░░░░  ░░░░░░  \r\n"
	                  + "                                                        \r\n"
	                  + "                                                        \r\n"
	                  + "                                                        ");
	            System.out.println("\n\n\n\n"
	                    + " ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______\r\n"
	                      + "/_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____/\n\n\n\n\n");
	            Thread.sleep(1000);
	            
	            
	          gcon.jing();
	          m.stp(23);

			} else if (select == 3) {
				// 종료
				System.out.println("\n\n"
		                 + " ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______\r\n"
		                   + "/_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____/\n\n\n\n\n");
				Thread.sleep(3000);
				System.out.println(""
	               		+ "                    /$$$$$$   /$$$$$$  /$$      /$$ /$$$$$$$$        /$$$$$$  /$$$$$$$$ /$$$$$$  /$$$$$$$  /$$$$$$$$               \r\n"
	               		+ "                   /$$__  $$ /$$__  $$| $$$    /$$$| $$_____/       /$$__  $$|__  $$__//$$__  $$| $$__  $$|__  $$__/               \r\n"
	               		+ " /$$$$ /$$$$ /$$$$| $$  \\__/| $$  \\ $$| $$$$  /$$$$| $$            | $$  \\__/   | $$  | $$  \\ $$| $$  \\ $$   | $$ /$$$$ /$$$$ /$$$$\r\n"
	               		+ "|____/|____/|____/| $$ /$$$$| $$$$$$$$| $$ $$/$$ $$| $$$$$         |  $$$$$$    | $$  | $$$$$$$$| $$$$$$$/   | $$|____/|____/|____/\r\n"
	               		+ " /$$$$ /$$$$ /$$$$| $$|_  $$| $$__  $$| $$  $$$| $$| $$__/          \\____  $$   | $$  | $$__  $$| $$__  $$   | $$ /$$$$ /$$$$ /$$$$\r\n"
	               		+ "|____/|____/|____/| $$  \\ $$| $$  | $$| $$\\  $ | $$| $$             /$$  \\ $$   | $$  | $$  | $$| $$  \\ $$   | $$|____/|____/|____/\r\n"
	               		+ "                  |  $$$$$$/| $$  | $$| $$ \\/  | $$| $$$$$$$$      |  $$$$$$/   | $$  | $$  | $$| $$  | $$   | $$                  \r\n"
	               		+ "                   \\______/ |__/  |__/|__/     |__/|________/       \\______/    |__/  |__/  |__/|__/  |__/   |__/                  \r\n"
	               		+ "                                                                                                                                   \r\n"
	               		+ "                                                             게임 스타트                                                              \r\n"
	               		+ "                                                                                                                                   ");
				System.out.println("\n\n\n\n"
		                 + " ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______ ______\r\n"
		                   + "/_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____//_____/\n\n\n\n\n\n");
			    
				
				break;
			} else {
				System.out.println("다시 선택해 주세요!.");
			}

		}
	}


}